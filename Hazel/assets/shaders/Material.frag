...
void main()
{
    ...
    vec3 lightDir = normalize(u_LightPosition - v_FragPosotion);
    float diffuseIntensity = max(dot(normal, lightDir), 0.0);
    vec3 diffuse = diffuseIntensity * u_LightAmbientIntensity * u_LightColor;
    
    //specular
    vec3 viewDir = normalize(u_ViewPosition - v_FragPosotion);
    vec3 reflectDir = reflect(-lightDir, normal);
    float specularIntensity = pow(max(dot(viewDir, reflectDir), 0.0), 32);
    vec3 specular = specularIntensity * u_LightDiffuseIntensity * u_LightColor;
